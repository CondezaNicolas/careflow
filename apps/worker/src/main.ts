...
class NoopNotificationGateway implements NotificationChannelGateway {
...
async function bootstrap() {
  ...
  const notificationsWorker = new NotificationOutboxWorker(pool, new NoopNotificationGateway(), {
    maxRetries: env.NOTIFICATIONS_MAX_RETRIES,
    backoffSeconds: env.NOTIFICATIONS_BACKOFF_SECONDS
  });

  try {
    logStructured("worker.run.started", { runId });
    const [calendarResult, notificationsResult] = await Promise.all([
      worker.processPending(),
      notificationsWorker.processPending()
    ]);
    logStructured("worker.run.completed", {
      runId,
      googleCalendar: calendarResult,
      notifications: notificationsResult,
      metricsSnapshot: getWorkerMetricsSnapshot()
    });
    if (calendarResult.failed > 0 || notificationsResult.failed > 0) {
      logStructured("worker.run.requires_attention", {
        runId,
        failedCalendarEvents: calendarResult.failed,
        failedNotificationEvents: notificationsResult.failed
      });
    }
  } finally {
    await pool.end();
